...
class UploadSalatlar extends StatefulWidget {
  static final String id = "upload_page";

  const UploadSalatlar({super.key});

  @override
  State<UploadSalatlar> createState() => _UploadSalatlarState();
}

class _UploadSalatlarState extends State<UploadSalatlar> {
  var isLoading = false;
  ...
  _apiUploadImage(String name, String content) {
    ...
    StorageService.uploadImage(_image!).then((img_url) => {
          _apiCreateSalat(name, content, img_url),
        });
  }

  _apiCreateSalat(String name, String content, String img_url) {
    var post = Post(name: name, caption: content, img_url: img_url);
    RTDBService.addSalatlar(post).then((value) => {
          _resAddPost(),
        });
  }
